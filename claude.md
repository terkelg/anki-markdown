...
`shiki-data.json` is generated by `scripts/generate.ts` from the installed `shiki` npm package (`bundledLanguagesInfo` / `bundledThemesInfo`). It contains the version, all available languages (including aliases), and all available themes. This JSON ships with the add-on.

At runtime, the version from `shiki-data.json` builds esm.sh URLs (`esm.sh/@shikijs/langs@3.22.0/...`), so the embedded list always matches what's available on the CDN. Bumping the shiki dependency in `package.json` and re-running `bun run generate` updates both the version and the language/theme lists together.
